    ...
    <view v-for="(item, index) in list" :key="index" class="activity-item">
      <view class="activity-item-row">
        ...
        <view class="activity-item-right">
          <view class="activity-item-content">{{ item.ActivityId }}</view>
          <view :class="['state', item.State === 2 ? 'gray' : '']">{{ item.Status }}</view>
        </view>
      </view>
      <view class="activity-item-row">
        <view class="activity-item-left">活动名称：</view>
        <view class="activity-item-right">
          <view class="activity-item-content">{{ item.ActivityName }}</view>
        </view>
      </view>
      <view class="activity-item-row">
      ...
      <view class="activity-item-row">
        <view class="activity-item-left">活动剩余：</view>
        <view class="activity-item-right">
          <view class="state">{{ item.RemainTime }}</view>
          <view class="hot-detai" @tap="handleDetail(item)">
            活动详情
...
export default {
  ...
  methods: {
    handleDetail(e) {
      uni.navigateTo({
        url: `./detail?Id=${e.ActivityId}&Status=${this.status}`
      })
    }
  }
}
