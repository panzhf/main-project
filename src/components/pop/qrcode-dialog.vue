<template>
  <view v-if="showMoneyDialog" class="money-dialog" @touchmove.stop="handleMove">
    <view class="dialog-mask" @tap="changeshowMoneyDialog" @touchmove.stop="handleMove"></view>
    <view class="dialog-content">
      <view class="dialog-title">
        <view class="follow">关注公众号 {{ publicCode.CompanyName }}</view>
        <view class="msg">（前往公众号-个人中心）</view>
      ...
      <view class="iconfont icon-guanbi1 dialog-close" @tap="changeshowMoneyDialog"></view>
    </view>
  </view>
</template>
<script>
...
export default {
  ...
  watch: {
    value(val) {
      this.showMoneyDialog = val
      if (val && !this.publicCode) {
        this.getData()
      }
    }
  },
  methods: {
    async getData() {
      apiCloud({
        url: '/api/v1/RedEnvelopeVoucherEvent/GetQRCode',
        method: 'post',
        data: JSON.stringify({})
      }).then(res => {
        if (this.$ck(res)) {
          this.publicCode = res.return_data
        }
      })
    },
