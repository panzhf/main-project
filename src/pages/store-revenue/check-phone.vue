<template>
  <view class="container" :style="themeVars">
    <view class="itemWrapper" style="background: #f7f7f7; line-height: 1.5">
      为了您的账户安全，需要向当前门店的店负责人手机号发送验证码，请注意短信查收。
    </view>
  ...
  methods: {
    SendPhoneMessage() {
      ...
      apiCloud({
        ...
        method: 'post',
        data: JSON.stringify({ Mobile: this.Mobile })
      })
        .then(res => {
    ...
    CherkPhoneMessage() {
      ...
      this.handleCherkPhone(this.openid, this.Headpath, this.NickName, this.CherkCode, 0)
    },
    countdown() {
      let nsecond = 60
      clearInterval(this.countdownInterval)
...
.loginInput {
  ...
  .g-message {
    ...
    border: 1px solid var(--theme-color, #ff5e33);
  }
}
.label {
  flex: 1;
