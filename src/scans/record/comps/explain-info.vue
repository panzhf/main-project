    ...
    <view class="desc-dialog flex flex-column" @tap.stop>
      ...
      <scroll-view class="content flex-1" :scroll-y="true" :enable-flex="true" :style="{ height: scrollViewHeight }">
        <view class="wrapper">
          ...
          <view class="text">{{ ruleInfo.title }}</view>
          <view class="label">活动时间</view>
          <view class="text">{{ ruleInfo.begintime }} 至 {{ ruleInfo.endtime }}</view>
          <template v-if="ruleInfo.rulecontent">
            <view class="label">活动规则</view>
            <u-parse class="text" :html="ruleInfo.rulecontent" :tag-style="parseStyle"></u-parse>
          </template>
        </view>
      </scroll-view>
      <view class="close-btn flex flex-center">
        <view class="iconfont icon-close2 fw-bold" @tap.stop="handleCloseDialog"></view>
      </view>
    </view>
  </u-mask>
</template>
